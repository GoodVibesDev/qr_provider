...
Future<Response> onRequest(RequestContext context, String id) async {
  final userId = id.split('.').firstOrNull;
  // final qr = Barcode.qrCode();
  // final svg = qr.toSvg(userId ?? '', width: 300, height: 300);

  final generator = QRGenerator();

  final pngBytes = await generator.generateImage(
    data: userId ?? '',
    scale: 10,
    padding: 2,
    qrVersion: 8,
...
class QRGenerator {
  ...
  late img.Color _bgColor;
  late img.Color _fgColor;
  ...
  Future<String> generate({
    ...
    img.ColorInt8 backgroundColor = img.ColorInt8.rgb(255, 255, 255);

    /// [foregroundColor] of a QR. Defaults to [Colors.black].
    img.ColorInt8 foregroundColor = img.ColorInt8.rgb(
      0,
      0,
      0,
    );
  ...
  Future<Uint8List> generateImage({
    ...
    try {
      return await _makeImage(saveFile: false);
    } catch (e) {
      throw 'Generate Image Error';
    }
  }

  Future<Uint8List> _makeImage({bool saveFile = true}) async {
    late QrCode qr;
    ...
    return _make(saveFile: saveFile);
  }

  Future<Uint8List> _make({bool saveFile = true}) async {
    /// Define Image
    ...
    for (int i = 0; i < enlargedData.length; i++) {
      ...
      for (int j = 0; j < enlargedData.length; j++) {
        ...
        if (d) {
          img.Color c = _fgColor;

          image.setPixel(i, j, c);
        } else {
          image.setPixel(i, j, _bgColor);
        }
      }
    }
